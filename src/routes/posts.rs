...
fn create_post_html_card(post: &Post) -> String {
    format!("<li 
            hx-boost='true'
            key={} 
            class=\"overflow-hidden bg-secondary rounded-xl border border-gray-200\"
        >
        <a href='{}'>
        <div
            class=\"w-full group relative cursor-pointer overflow-hidden bg-secondary px-6 pt-10 pb-8 shadow-xl ring-1 ring-gray-900/5 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl sm:mx-auto sm:rounded-lg sm:px-10\"
        >
            ...
            <div class=\"relative z-10 mx-auto max-w-md\">
                ...
                >
                    <h3 class='truncate text-3xl font-extrabold text-white'>{}</h3>
                ...
                >
                    <input id='id' name='id' class='invisible' value='{}'></input>
                    <input id='title' name='title' class='invisible' value='{}'></input>
                    <input id='author' name='author' class='invisible' value='{}'></input>
                    <input id='url' name='url' class='invisible' value='{}'></input>
                    <input id='timestamp' name='timestamp' class='invisible' value='{}'></input>
                    <button
                      id='save-btn-{}'
                      type='submit'
                      class='rounded-md bg-primary px-3.5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-white/50 transition-all duration-300 group-hover:bg-secondary'
                    >
                        <span id='save-post-text'>Save Post</span>
                        <div id='spinner' style='display: none;'>
                            <svg aria-hidden='true' role='status' class='inline w-4 h-4 mr-3 text-white text-center animate-spin' viewBox='0 0 100 101' fill='none' xmlns='http://www.w3.org/2000/svg'>
